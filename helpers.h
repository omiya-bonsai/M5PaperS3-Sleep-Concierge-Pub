...
void drawGauge(int x, int y, int w, int h, float value, float min_val, float max_val, const char* label) {
  ...
  canvas.drawString(String(value, 0), x + w, y + h / 2);
}
...
void drawCalendar(int tx, int ty, int year, int month, int day) {
  ...
  canvas.drawString(String(year) + "年 " + String(month) + "月", tx + (38 * 7) / 2, ty);

  ty += 40;
  canvas.setFont(&fonts::lgfxJapanGothic_20);
  for (int i = 0; i < 7; i++) {
    canvas.drawString(week_days[i], tx + i * 38 + 19, ty);
  }
  ...
  for (int row = 0; row < 6; row++) {
    for (int col = 0; col < 7; col++) {
      if ((row == 0 && col < first_day_of_week) || current_day > days_in_month[month - 1]) {
        ...
        int day_x = tx + col * 38 + 19;
        int day_y = ty + row * 35 + 17;

        if (current_day == day) {
          canvas.fillCircle(day_x, day_y, 17, BLACK);
...
/**
 * @brief 現在の日付から二十四節気を判定し、メッセージを生成します。
 */
...
void drawScreen() {
  ...
  current_y += 30;
  int gauge_h = 28;
  int gauge_w = M5.Display.width() - (H_MARGIN * 2);
  drawGauge(H_MARGIN, current_y, gauge_w, gauge_h, indoor_temp, 15, 30, "温度");
  current_y += gauge_h + 15;
  drawGauge(H_MARGIN, current_y, gauge_w, gauge_h, indoor_hum, 40, 70, "湿度");
  current_y += gauge_h + 15;
  drawGauge(H_MARGIN, current_y, gauge_w, gauge_h, indoor_co2, 400, 1500, "CO2");

  current_y += gauge_h + 30;
  canvas.drawFastHLine(H_MARGIN, current_y, M5.Display.width() - (H_MARGIN * 2), LIGHTGREY);

  // Block 4: Seasonal Info & Calendar
  current_y += 30;
  drawCalendar((M5.Display.width() - (38 * 7)) / 2, current_y, timeinfo.tm_year + 1900, month, timeinfo.tm_mday);
  current_y += 260;  // カレンダーの高さ分を確保
